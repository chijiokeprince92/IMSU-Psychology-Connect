    ...
    News.findOneAndUpdate({
        ...
        function(err) {
            if (err) {
                ...
                req.flash('den', 'You liked this news');
    ...
    News.findOneAndUpdate({
        ...
        function(err) {
            if (err) {
                ...
                req.flash('den', 'You disliked this news');
                res.redirect('/studentgetlastnews');
            }
        }
    );
...
exports.post_comment_news = (req, res, next) => {
    ...
    News.findByIdAndUpdate(
        ...
        (err, cement) => {
            ...
            console.log('final result: ', cement.comments.length);
...
exports.news_reply_comment = (req, res, next) => {
    ...
    News.findOneAndUpdate({
        ...
        function(err, cement) {
            ...
            req.io.emit('reply_message', req.body.reply);
            res.json({
                status: 200,
                data: cement
            });
...
exports.my_result = (req, res, next) => {
    StudentSigns.findOne({
        ...
        if (req.session.student.level < pupil.level) {
            req.flash('message', "This student is your senior colleague, so you can't view their result");
